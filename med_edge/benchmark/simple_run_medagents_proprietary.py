...
Usage:
    python -m med_edge.benchmark.simple_run_medagents_proprietary --model "openai/gpt-4" --output-dir ./results
    python -m med_edge.benchmark.simple_run_medagents_proprietary --model "..." --output-dir ./results --configs MMLU MedMCQA
    python -m med_edge.benchmark.simple_run_medagents_proprietary --help
"""
...
def run_single_config(dataset_config, config, output_dir, limit):
    """Run benchmark for a single config with resume support."""
    logger.info(f"Starting benchmark: {config['model']} on medagents-benchmark/{dataset_config}")
    if limit:
        ...
        for idx, sample in enumerate(test_data):
            ...
            if completed_count % SAVE_EVERY == 0:
                try:
                    ...
                    logger.error(f"Checkpoint save failed (continuing): {save_err}")
...
def main(model, configs, limit, output_dir, env_file, verbose):
    """Run medagents-benchmark with proprietary models (OpenAI, Anthropic, etc.)."""
    ...
    logger.info("=" * 80)
    logger.info(f"Running medagents-benchmark configs with proprietary model")
    ...
    for idx, dataset_config in enumerate(configs_to_run, 1):
        ...
        try:
            df = run_single_config(dataset_config, config, output_dir, limit)
            accuracy = df['is_correct'].mean()
